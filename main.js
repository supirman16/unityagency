...
import { setupUIForRole, showSection, getFirstVisibleSection, applyTheme, showNotification, showButtonLoader, hideButtonLoader, openSettingsModal, openRekapModal, openHostModal, openTiktokModal, openUserModal, openDetailRekapModal, handleDeleteHost, handleDeleteTiktok, handleDeleteRekap, handleDeleteUser } from './ui.js';
...
function setupEventListeners() {
    ...
    btnConfirmDelete.addEventListener('click', async (e) => {
        ...
        try {
            if (state.itemToDelete.type === 'host') {
                ({ error } = await supabase.from('hosts').delete().eq('id', state.itemToDelete.id));
            } else if (state.itemToDelete.type === 'rekap') {
                ({ error } = await supabase.from('rekap_live').delete().eq('id', state.itemToDelete.id));
            } else if (state.itemToDelete.type === 'tiktok') {
                ({ error } = await supabase.from('tiktok_accounts').delete().eq('id', state.itemToDelete.id));
            } else if (state.itemToDelete.type === 'user') {
                ({ data, error } = await supabase.functions.invoke('delete-user', {
    ...
    formHost.addEventListener('submit', async (e) => {
        ...
        try {
            if (hostId) {
                ({ error } = await supabase.from('hosts').update(hostData).eq('id', hostId));
            } else {
                ({ error } = await supabase.from('hosts').insert(hostData));
            }
            if (error) throw error;
    ...
    formTiktok.addEventListener('submit', async (e) => {
        ...
        try {
            if (tiktokId) {
                ({ error } = await supabase.from('tiktok_accounts').update(tiktokData).eq('id', tiktokId));
            } else {
                ({ error } = await supabase.from('tiktok_accounts').insert(tiktokData));
            }
            if (error) throw error;
    ...
    formRekap.addEventListener('submit', async (e) => {
        ...
        try {
            if (rekapId) {
                ({ error } = await supabase.from('rekap_live').update(rekapData).eq('id', rekapId));
            } else {
                ({ error } = await supabase.from('rekap_live').insert(rekapData));
            }
            if (error) throw error;
    ...
    document.getElementById('btn-confirm-import').addEventListener('click', async (e) => {
        ...
        try {
            if (validData.length > 0) {
                const { error } = await supabase.from('rekap_live').insert(validData);
                if (error) throw error;
