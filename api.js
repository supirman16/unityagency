...
export async function fetchData() {
    const { data: hostsData, error: hostsError } = await supabase.from('hosts').select('*');
    if (hostsError) console.error('Error fetching hosts:', hostsError);
    else state.hosts = hostsData;

    const { data: tiktokData, error: tiktokError } = await supabase.from('tiktok_accounts').select('*');
    if (tiktokError) console.error('Error fetching tiktok accounts:', tiktokError);
    else state.tiktokAccounts = tiktokData;
    
    const { data: rekapData, error: rekapError } = await supabase.from('rekap_live').select('*');
    ...
    if (state.currentUser?.user_metadata?.role === 'superadmin') {
        const { data: usersData, error: usersError } = await supabase.functions.invoke('list-all-users');
