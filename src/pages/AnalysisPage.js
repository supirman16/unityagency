...
export default function AnalysisPage() {
    const { data, session } = useContext(AppContext);
    const [currentDate, setCurrentDate] = useState(new Date());
    
    const isSuperAdmin = session.user.user_metadata?.role === 'superadmin';
    const [selectedHostId, setSelectedHostId] = useState(isSuperAdmin ? (data.hosts[0]?.id || '') : session.user.user_metadata.host_id);

    const performance = useMemo(() => {
        if (!selectedHostId) return {};
        return calculateMonthlyPerformance(selectedHostId, currentDate.getFullYear(), currentDate.getMonth(), data.rekapLive);
...
function Calendar({ currentDate, selectedHostId }) {
    ...
    return (
        <div className="mt-8 bg-white dark:bg-stone-800 rounded-xl shadow-sm border border-stone-100 dark:border-stone-700 p-4">
            ...
            <div className="block md:grid md:grid-cols-7 gap-1">
                {emptyStartDays.map((_, i) => <div key={`empty-${i}`} className="hidden md:block border border-stone-200 dark:border-stone-700 bg-stone-50 dark:bg-stone-800/50 rounded-md"></div>)}
                {daysArray.map(day => {
                    ...
                    return (
                        <React.Fragment key={day}>
                            ...
                            {/* Mobile View */}
                            <div className="flex md:hidden calendar-day-mobile border-stone-200 dark:border-stone-700">
...
function calculateMonthlyPerformance(hostId, year, month, rekapLive) {
    ...
    const hostRekaps = rekapLive.filter(r => {
        ...
        return r.host_id === parseInt(hostId) && recYear === year && (recMonth - 1) === month && r.status === 'approved';
    });
